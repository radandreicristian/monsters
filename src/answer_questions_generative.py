...
def answer_questions(image_generation_model_id: str,
    ...
    for concept_name, concept_formulation in concepts.items():
        ...
        images_root_path = f"data/biased_images/{model_name}/{concept_name}/direct"
        
        if not os.path.isdir(images_root_path):
            logger.warning(f"Skipping {concept_name}. Missing folder: {images_root_path}")
            continue
